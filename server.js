  ...
  http
    .createServer({}, async (request, response) => {
      ...
      try {
        ...
        if (request.method === 'GET' && requestUrl.pathname === '/') {
          ...
          const attendee = await chime
            .createAttendee({
              ...
              ExternalUserId: `${uuidv4().substring(0, 8)}#${requestUrl.query.name}`.substring(
                0,
                64
              ),
            })
        ...
        } else if (request.method === 'GET' && requestUrl.pathname === '/recording') {
          axios
            .post(
              `https://4x9i6jrsug.execute-api.ap-southeast-1.amazonaws.com/Prod/recording?recordingAction=${
                requestUrl.query.recordingAction
              }&meetingURL=${encodeURIComponent(requestUrl.query.meetingURL)}&taskId=${
                requestUrl.query.taskId
              }`
            )
            .then(data => {
              respond(
                ...
                200,
                'application/json',
                JSON.stringify({ taskId: data.data }),
                true
              );
            });
        } else {
