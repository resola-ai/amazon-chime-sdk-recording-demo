...
class TestSound {
  ...
  async init(): Promise<void> {
    ...
    gainNode.gain.linearRampToValueAtTime(
      0,
      startTime + this.rampSec * 2 + this.durationSec
    );
    ...
    new TimeoutScheduler(
      (this.rampSec * 2 + this.durationSec + 1) * 1000
    ).start(() => {
      audioContext.close();
    });
  }
...
export class DemoMeetingApp
  ...
  activeSpeakerHandler:
    | undefined
    | ((attendeeIds: string[]) => void) = undefined;
  ...
  fatal(e: Error | string): void {
    ...
    console.error(
      "Fatal error: this was going to be caught, but should not have been thrown.",
      e
    );
  ...
  initParameters(): void {
    ...
    if (meeting) {
      (document.getElementById(
        "inputMeeting"
      ) as HTMLInputElement).value = meeting;
  ...
  async initVoiceFocus(): Promise<void> {
    ...
    if (!this.enableWebAudio) {
      logger.info(
        "[DEMO] Web Audio not enabled. Not checking for Amazon Voice Focus support."
      );
      return;
    }

    try {
      this.supportsVoiceFocus = await VoiceFocusDeviceTransformer.isSupported(
        VOICE_FOCUS_SPEC,
        {
          logger,
        }
  ...
  private async onVoiceFocusSettingChanged(): Promise<void> {
    this.log(
      "[DEMO] Amazon Voice Focus setting toggled to",
      this.enableVoiceFocus
    );
  ...
  initEventListeners(): void {
    ...
    document
      ...
      .addEventListener("submit", (e) => {
        e.preventDefault();
        this.meeting = (document.getElementById(
          "inputMeeting"
        ) as HTMLInputElement).value;
        this.name = (document.getElementById(
          "inputName"
        ) as HTMLInputElement).value;
        this.region = (document.getElementById(
          "inputRegion"
        ) as HTMLInputElement).value;
        this.enableSimulcast = (document.getElementById(
          "simulcast"
        ...
        this.enableWebAudio = (document.getElementById(
          "webaudio"
        ) as HTMLInputElement).checked;
        ...
        AsyncScheduler.nextTick(
          async (): Promise<void> => {
            ...
            try {
              ...
              (document.getElementById(
                "failed-meeting-error"
              ) as HTMLDivElement).innerText = error.message;
              this.switchToFlow("flow-failed-meeting");
              return;
            }
            const sipUriElement = document.getElementById(
              "sip-uri"
            ) as HTMLInputElement;
    ...
    (document.getElementById(
      "add-voice-focus"
    ) as HTMLInputElement).addEventListener("change", (e) => {
    ...
    buttonRecord.addEventListener("click", (_e) => {
      axios
        .get(
          `${DemoMeetingApp.BASE_URL}recording?=${encodeURIComponent(
    ...
    buttonMeetingEnd.addEventListener("click", (_e) => {
      const confirmEnd =
        new URL(window.location.href).searchParams.get("confirm-end") ===
        "true";
      const prompt =
  ...
  logPPS() {
    ...
    setInterval(async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const stats = (await this.audioVideo.getRTCPeerConnectionStats()) as RTCStatsReport &
        RTCStats &
        Map<string, any>;
  ...
  metricsDidReceive(clientMetricReport: ClientMetricReport): void {
    ...
    if (
      ...
      (document.getElementById(
        "video-uplink-bandwidth"
      ) as HTMLSpanElement).innerText = "Available Uplink Bandwidth: Unknown";
    }
    ...
    if (
      ...
      (document.getElementById(
        "video-downlink-bandwidth"
      ) as HTMLSpanElement).innerText = "Available Downlink Bandwidth: Unknown";
    }
    ...
    if (rawStats) {
      rawStats.forEach((report) => {
        if (report.type === "local-candidate") {
          this.log(
            `Local WebRTC Ice Candidate stats: ${JSON.stringify(report)}`
          );
          const relayProtocol = report.relayProtocol;
          if (typeof relayProtocol === "string") {
            if (relayProtocol === "udp") {
              this.log(
                `Connection using ${relayProtocol.toUpperCase()} protocol`
              );
            } else {
              this.log(
                `Connection fell back to ${relayProtocol.toUpperCase()} protocol`
              );
            }
          }
        }
  ...
  async initializeMeetingSession(
    ...
    const consoleLogger = (this.meetingLogger = new ConsoleLogger(
      "SDK",
      logLevel
    ));
    ...
    if (
      ...
      this.meetingSession.audioVideo.setAudioProfile(
        AudioProfile.fullbandSpeechMono()
      );
      this.meetingSession.audioVideo.setContentAudioProfile(
        AudioProfile.fullbandSpeechMono()
      );
    } else if (
      (document.getElementById(
      ...
      this.meetingSession.audioVideo.setAudioProfile(
        AudioProfile.fullbandMusicMono()
      );
      this.meetingSession.audioVideo.setContentAudioProfile(
        AudioProfile.fullbandMusicMono()
      );
  ...
  async joinMeeting(): Promise<any> {
    const response = await fetch(
      `${DemoMeetingApp.BASE_URL}join?title=${encodeURIComponent(
        this.meeting
      )}&name=${encodeURIComponent(this.name)}&region=${encodeURIComponent(
        this.region
      )}`,
      {
        method: "POST",
  ...
  async endMeeting(): Promise<any> {
    await fetch(
      `${DemoMeetingApp.BASE_URL}end?title=${encodeURIComponent(this.meeting)}`,
      {
        method: "POST",
      }
    );
  ...
  setupDeviceLabelTrigger(): void {
    ...
    this.audioVideo.setDeviceLabelTrigger(
      async (): Promise<MediaStream> => {
        if (this.isRecorder() || this.isBroadcaster()) {
          throw new Error(
            "Recorder or Broadcaster does not need device labels"
          );
        }
  ...
  private async reselectAudioInputDevice(): Promise<void> {
    ...
    if (current instanceof VoiceFocusTransformDevice) {
      ...
      const device = await this.audioInputSelectionWithOptionalVoiceFocus(
        intrinsic
      );
  ...
  async openVideoInputFromSelection(
    ...
    const device = await this.videoInputSelectionToDevice(
  ...
  private async audioInputSelectionToIntrinsicDevice(
    value: string
  ): Promise<Device> {
    if (this.isRecorder() || this.isBroadcaster()) {
      return null;
  ...
  private async createVoiceFocusDevice(
    inner: Device
  ): Promise<VoiceFocusTransformDevice | Device> {
    if (!this.supportsVoiceFocus) {
      return inner;
    }
    ...
    if (this.voiceFocusDevice) {
      ...
      return (this.voiceFocusDevice = await this.voiceFocusDevice.chooseNewInnerDevice(
        inner
      ));
    }
    ...
    try {
      ...
      const vf: VoiceFocusTransformDevice = await transformer.createTransformDevice(
        inner
      );
      if (vf) {
  ...
  private async playToStream(
    videoFile: HTMLVideoElement
  ): Promise<MediaStream> {
    await videoFile.play();
  ...
  private updateContentShareDropdown(enabled: boolean): void {
    ...
    document.getElementById(
      "dropdown-item-content-share-stop"
  ...
  isBroadcaster(): boolean {
    return (
      new URL(window.location.href).searchParams.get("broadcast") === "true"
    );
  }
  ...
  audioVideoDidStop(sessionStatus: MeetingSessionStatus): void {
    ...
    const cleanUpResources = async () => {
      ...
      this.audioVideo.realtimeUnsubscribeToAttendeeIdPresence(
  ...
  remoteVideoSourcesDidChange(videoSources: VideoSource[]): void {
    this.log(
      `available remote video sources changed: ${JSON.stringify(videoSources)}`
    );
  }
}
